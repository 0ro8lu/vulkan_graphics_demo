...
private:
  ...
  void cleanup()
  {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
      ...
      vmaDestroyBuffer(
        allocator, dynUniformBuffers[i], dynUniformBuffersAllocation[i]);
      vmaDestroyBuffer(
    ...
    vkDestroyDescriptorSetLayout(
      logicalDevice, bindlessDescriptorSetLayout, nullptr);
  ...
  void createInstance()
  {
    ...
    // configure MoltenVK a bit so i don't get the absolutely miniscule
    // maxPerStageDescriptorUpdateAfterBindSamplers limit (16). this should do
    // some Metal magic and enable it to go much higher.
  ...
  void createDescriptorSetLayout()
  {
    ...
    dynUBOLayoutBinding.descriptorType =
    ...
    // diffuseBindless.descriptorCount =
    ...
    // specularBindless.descriptorCount =
    // properties.limits.maxDescriptorSetSampledImages;
    ...
    std::array<VkDescriptorSetLayoutBinding, 2> bindlessLayoutBinding = {
      diffuseBindless, specularBindless
    };

    VkDescriptorSetLayoutCreateInfo bindlessLayoutInfo{};
    bindlessLayoutInfo.sType =
      VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
    bindlessLayoutInfo.bindingCount =
      static_cast<uint32_t>(bindlessLayoutBinding.size());
  ...
  {
    createPipeline("../shaders/bindless-textures/built/texture_vert.spv",
                   "../shaders/bindless-textures/built/texture_frag.spv",
                   mainPipelineLayout,
                   mainGraphicsPipeline);
    createPipeline("../shaders/bindless-textures/built/light_cube_vert.spv",
                   "../shaders/bindless-textures/built/light_cube_frag.spv",
                      ...
                      VkPipeline& pipeline)
  {
    ...
    std::array<VkDescriptorSetLayout, 2> layouts = {
      globalDescriptorSetLayout, bindlessDescriptorSetLayout
    };
  ...
  void prepareTextures()
  {
    ...
    createTextureImage(
      "../textures/diffuse.png", diffuseTextureImage, diffuseTextureAllocation);
    ...
    createTextureImage("../textures/specular.png",
                       specularTextureImage,
                       specularTextureAllocation);
                          ...
                          VmaAllocation& textureAllocation)
  {
    ...
    void* data = createBuffer(imageSize,
  ...
  void createTextureImageView(const VkImage& textureImage,
  ...
  void createTextureSampler(VkSampler& texureSampler)
  {
    ...
    if (vkCreateSampler(logicalDevice, &samplerInfo, nullptr, &texureSampler) !=
        VK_SUCCESS) {
  ...
  void createVertexBuffer()
  {
    ...
    VkDeviceMemory stagingBufferMemory;
    VmaAllocation stagingBufferAllocation;
    void* data = createBuffer(bufferSize,
                              VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                              BufferType::STAGING_BUFFER,
                              stagingBuffer,
                              stagingBufferAllocation);
  ...
  //     VkDeviceMemory stagingBufferMemory;
  //     VmaAllocation stagingBufferAllocation;
  //     void* data = createBuffer2(bufferSize,
  //     VK_BUFFER_USAGE_TRANSFER_SRC_BIT, BufferType::STAGING_BUFFER,
  //     stagingBuffer, stagingBufferAllocation);

  //     memcpy(data, indices.data(), (size_t) bufferSize);

  //     createBuffer2(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT |
  //     VK_BUFFER_USAGE_INDEX_BUFFER_BIT, BufferType::GPU_BUFFER, indexBuffer,
  ...
  void createUniformBuffers()
  {
    ...
    size_t alignMod =
      static_cast<size_t>(gpuProperties.limits.minUniformBufferOffsetAlignment);
    dynamicAlignmentUBO =
      (sizeof(DynUniformBufferObject) / alignMod) * alignMod;
    if (sizeof(DynUniformBufferObject) % alignMod > 0) {
      dynamicAlignmentUBO += alignMod;
    }

    VkDeviceSize dynBufferSize =
      (OBJECT_COUNT + NR_POINT_LIGHTS) * dynamicAlignmentUBO;
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
      staticUniformBuffersMapped[i] =
        createBuffer(staticBufferSize,
                     VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                     BufferType::STAGING_BUFFER,
                     uniformBuffers[i],
                     uniformBuffersAllocation[i]);
      dynUniformBuffersMapped[i] =
        createBuffer(dynBufferSize,
                     VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                     BufferType::STAGING_BUFFER,
                     dynUniformBuffers[i],
                     dynUniformBuffersAllocation[i]);
    ...
    std::array<DynUniformBufferObject, OBJECT_COUNT + NR_POINT_LIGHTS>
      dynUBOs{};
    ...
    for (int i = 0; i < OBJECT_COUNT; i++) {
      ...
      float angle = 20.0f * i;
      dynUBOs[i].model = glm::rotate(
        dynUBOs[i].model, glm::radians(angle), glm::vec3(1.0f, 0.3f, 0.5f));
    }

    for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
      for (int j = 0; j < OBJECT_COUNT; j++) {
        uint8_t* mappedUniform =
          reinterpret_cast<uint8_t*>(dynUniformBuffersMapped[i]);
        memcpy(&mappedUniform[j * dynamicAlignmentUBO],
               &dynUBOs[j],
               sizeof(DynUniformBufferObject));
      }
    }
    ...
    for (int i = OBJECT_COUNT; i < (OBJECT_COUNT + NR_POINT_LIGHTS); i++) {
      dynUBOs[i].model = glm::mat4(1.0);
      dynUBOs[i].model =
        glm::translate(dynUBOs[i].model, pointLightPositions[i - OBJECT_COUNT]);
  ...
  void createDescriptorPool()
  {
    ...
    // bindlessPoolSizes[0].descriptorCount =
    // properties.limits.maxDescriptorSetSampledImages;
    ...
    bindlessPoolInfo.poolSizeCount =
      static_cast<uint32_t>(bindlessPoolSizes.size());
    // bindlessPoolInfo.flags =
...
#ifdef __APPLE__
    // SRS - increase the per-stage descriptor samplers limit on macOS
    ...
    if (vkCreateDescriptorPool(
          logicalDevice, &bindlessPoolInfo, nullptr, &bindlessDescriptorPool) !=
        VK_SUCCESS) {
      throw std::runtime_error("failed to create descriptor pool!");
    }
  }
  ...
  void createDescriptorSets()
  {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
      ...
      // diffuseImageInfo.imageLayout =
      // VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL; diffuseImageInfo.imageView =
      // diffuseTextureImageView; diffuseImageInfo.sampler =
      // diffuseTextureSampler;

      // VkDescriptorImageInfo specularImageInfo{};
      // specularImageInfo.imageLayout =
      // VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL; specularImageInfo.imageView =
      // specularTextureImageView; specularImageInfo.sampler =
    ...
    std::vector<VkDescriptorSetLayout> bindlessLayouts(
      MAX_FRAMES_IN_FLIGHT, bindlessDescriptorSetLayout);
    ...
    bindlessAllocateInfo.descriptorSetCount =
  ...
  void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
  {
    ...
    VkDescriptorSet descSets[] = { globalDescriptorSets[currentFrame],
                                   bindlessDescriptorSets[currentFrame] };

    for (int i = 0; i < OBJECT_COUNT; i++) {
      uint32_t dynamicOffsetUBO =
        static_cast<uint32_t>(i * dynamicAlignmentUBO);
