...
private:
    ...
    void cleanup() {
        ...
        vkDestroyDescriptorPool(logicalDevice, bindlessDescriptorPool, nullptr);

        // destroy diffuse texture
        vkDestroySampler(logicalDevice, diffuseTextureSampler, nullptr);
        vkDestroyImageView(logicalDevice, diffuseTextureImageView, nullptr);

        // destroy specular texture
        vkDestroySampler(logicalDevice, specularTextureSampler, nullptr);
        vkDestroyImageView(logicalDevice, specularTextureImageView, nullptr);
        ...
        vkDestroyDescriptorSetLayout(logicalDevice, bindlessDescriptorSetLayout, nullptr);
    ...
    void createInstance() {
        ...
        //configure MoltenVK a bit so i don't get the absolutely miniscule maxPerStageDescriptorUpdateAfterBindSamplers limit (16).
        //this should do some Metal magic and enable it to go much higher.
        ...
        // debugCreateInfo.pNext = &layer_settings_create_info;
    ...
    void createDescriptorSetLayout() {
        ...
        directionalLightLayoutBinding.binding = 2;
        ...
        pointLightLayoutBinding.binding = 3;
        ...
        spotLightLayoutBinding.binding = 4;
        ...
        if (vkCreateDescriptorSetLayout(logicalDevice, &layoutInfo, nullptr, &globalDescriptorSetLayout) != VK_SUCCESS) {
        ...
        VkDescriptorSetLayoutBinding diffuseBindless{};
        diffuseBindless.binding = 0;
        diffuseBindless.descriptorCount = 9;
        // diffuseBindless.descriptorCount = properties.limits.maxDescriptorSetSampledImages;
        diffuseBindless.descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
        diffuseBindless.pImmutableSamplers = nullptr;
        diffuseBindless.stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;

        VkDescriptorSetLayoutBinding specularBindless{};
        specularBindless.binding = 1;
        specularBindless.descriptorCount = 9;
        // specularBindless.descriptorCount = properties.limits.maxDescriptorSetSampledImages;
        specularBindless.descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
        specularBindless.pImmutableSamplers = nullptr;
        specularBindless.stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;

        std::array<VkDescriptorSetLayoutBinding, 2> bindlessLayoutBinding = {diffuseBindless, specularBindless};
        ...
        bindlessLayoutInfo.bindingCount = static_cast<uint32_t>(bindlessLayoutBinding.size());
        bindlessLayoutInfo.pBindings = bindlessLayoutBinding.data();
        ...
        extended_info.bindingCount = static_cast<uint32_t>(bindlessLayoutBinding.size());
        ...
        if (vkCreateDescriptorSetLayout(logicalDevice, &bindlessLayoutInfo, nullptr, &bindlessDescriptorSetLayout) != VK_SUCCESS) {
            throw std::runtime_error("failed to create descriptor set layout!");
        }
    }

    void createGraphicsPipelines() {
        createPipeline("../shaders/bindless-textures/built/texture_vert.spv", "../shaders/bindless-textures/built/texture_frag.spv", mainPipelineLayout, mainGraphicsPipeline);
        createPipeline("../shaders/bindless-textures/built/light_cube_vert.spv", "../shaders/bindless-textures/built/light_cube_frag.spv", lightCubePipelineLayout, lightCubePipeline);
    }
    ...
    void createPipeline(std::string vertexShader, std::string fragmentShader, VkPipelineLayout& pipelineLayout, VkPipeline& pipeline) {
        ...
        std::array<VkDescriptorSetLayout, 2> layouts = {globalDescriptorSetLayout, bindlessDescriptorSetLayout};
    ...
    void createTextureSampler(VkSampler& texureSampler) {
        ...
        if (vkCreateSampler(logicalDevice, &samplerInfo, nullptr, &texureSampler) != VK_SUCCESS) {
    ...
    void createUniformBuffers() {
        ...
        VkDeviceSize staticBufferSize = sizeof(UniformBufferObject);
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            staticUniformBuffersMapped[i] = createBuffer(staticBufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, BufferType::STAGING_BUFFER, uniformBuffers[i], uniformBuffersAllocation[i]);
            ...
            //for the lights
            directionalLightBuffersMapped[i] = createBuffer(directionalLightBufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, BufferType::STAGING_BUFFER, directionalLightBuffers[i], directionalLightBuffersAllocation[i]);
        ...
        // directional lights
        for(int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            uint8_t* mappedUniform = reinterpret_cast<uint8_t*>(directionalLightBuffersMapped[i]);
    ...
    void createDescriptorPool() {
        ...
        if (vkCreateDescriptorPool(logicalDevice, &poolInfo, nullptr, &globalDescriptorPool) != VK_SUCCESS) {
            throw std::runtime_error("failed to create descriptor pool!");
        }

        // ---------- bindless textures descriptor pool ---------- 
        std::array<VkDescriptorPoolSize, 1> bindlessPoolSizes{};

        VkPhysicalDeviceProperties properties{};
        vkGetPhysicalDeviceProperties(physicalDevice, &properties);

        bindlessPoolSizes[0].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
        // bindlessPoolSizes[0].descriptorCount = properties.limits.maxDescriptorSetSampledImages;
        bindlessPoolSizes[0].descriptorCount = 32;
        ...
        bindlessPoolInfo.maxSets = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT);

        std::cout << "\n\n\n\nDIOMERDAAAAAAAAAAAAAAAA\n\n\n\n" << std::endl;
// #if (defined(VK_USE_PLATFORM_MACOS_MVK) || defined(VK_USE_PLATFORM_METAL_EXT))
        ...
        if (vkCreateDescriptorPool(logicalDevice, &bindlessPoolInfo, nullptr, &bindlessDescriptorPool) != VK_SUCCESS) {
            throw std::runtime_error("failed to create descriptor pool!");
        }
    }
    ...
    void createDescriptorSets() {
        ...
        if (vkAllocateDescriptorSets(logicalDevice, &allocInfo, globalDescriptorSets.data()) != VK_SUCCESS) {
            throw std::runtime_error("failed to allocate descriptor sets!");
        }

        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            VkDescriptorBufferInfo staticBufferInfo{};
            staticBufferInfo.buffer = uniformBuffers[i];
            staticBufferInfo.offset = 0;
            staticBufferInfo.range = sizeof(UniformBufferObject);
            ...
            descriptorWrites[4].dstSet = globalDescriptorSets[i];
            descriptorWrites[4].dstBinding = 4;
            descriptorWrites[4].dstArrayElement = 0;
            descriptorWrites[4].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
            descriptorWrites[4].descriptorCount = 1;
            descriptorWrites[4].pBufferInfo = &spotLightBufferInfo;

            vkUpdateDescriptorSets(logicalDevice, static_cast<uint32_t>(descriptorWrites.size()), descriptorWrites.data(), 0, nullptr);
        }

        std::vector<VkDescriptorSetLayout> bindlessLayouts(MAX_FRAMES_IN_FLIGHT, bindlessDescriptorSetLayout);
        ...
        bindlessAllocateInfo.descriptorSetCount = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT);
        bindlessAllocateInfo.pSetLayouts = bindlessLayouts.data();

        bindlessDescriptorSets.resize(MAX_FRAMES_IN_FLIGHT);

        if (vkAllocateDescriptorSets(logicalDevice, &bindlessAllocateInfo, bindlessDescriptorSets.data()) != VK_SUCCESS) {
            throw std::runtime_error("failed to allocate descriptor sets!");
        }
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            ...
            descriptorWrites[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
            descriptorWrites[0].dstSet = bindlessDescriptorSets[i];
            descriptorWrites[0].dstBinding = 0;
            descriptorWrites[0].dstArrayElement = 0;
            ...
            descriptorWrites[1].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
            descriptorWrites[1].dstSet = bindlessDescriptorSets[i];
            descriptorWrites[1].dstBinding = 1;
            descriptorWrites[1].dstArrayElement = 0;
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        vkCmdBeginRenderPass(commandBuffer, &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);
            ...
            PushConstant pc;
            pc.textureRID = 0;
            vkCmdPushConstants(commandBuffer, mainPipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, 4, &pc);

            VkDescriptorSet descSets[] = {globalDescriptorSets[currentFrame], bindlessDescriptorSets[currentFrame]};

            for(int i = 0; i < OBJECT_COUNT; i++) {
                uint32_t dynamicOffsetUBO = static_cast<uint32_t>(i * dynamicAlignmentUBO); 

                vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, mainPipelineLayout, 0, 2, descSets, 1, &dynamicOffsetUBO);
            ...
            for(int i = OBJECT_COUNT; i < (OBJECT_COUNT + NR_POINT_LIGHTS); i++) {
                ...
                vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, lightCubePipelineLayout, 0, 1, &globalDescriptorSets[currentFrame], 1, &dynamicOffsetUBO);
                vkCmdDraw(commandBuffer, 36, 1, 0, 0);
            }
