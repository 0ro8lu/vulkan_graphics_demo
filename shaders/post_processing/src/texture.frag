...
vec3 CalcPointLight(vec3 lightPos, vec3 normal, vec3 fragPos, vec3 viewDir)
{
    ...
    // combine results
    vec3 resultAmbient = ambient * vec3(texture(diffuseTexSampler, fragTexCoord));
    vec3 resultDiffuse = diffuse * diff * vec3(texture(diffuseTexSampler, fragTexCoord));
    vec3 resultSpecular = specular * spec * vec3(texture(specularTexSampler, fragTexCoord));
