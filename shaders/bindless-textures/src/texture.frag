...
vec3 CalcPointLight(vec3 lightPos, vec3 normal, vec3 fragPos, vec3 viewDir)
{
    ...
    // combine results
    vec3 ambient = ambient * vec3(texture(diffuseTexSampler[nonuniformEXT(textureID)], fragTexCoord));
    vec3 diffuse = diffuse * diff * vec3(texture(diffuseTexSampler[nonuniformEXT(textureID)], fragTexCoord));
    vec3 specular = specular * spec * vec3(texture(specularTexSampler[nonuniformEXT(textureID)], fragTexCoord));
    ambient *= attenuation;
    diffuse *= attenuation;
